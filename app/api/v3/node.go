...
func (h Handler) NodeIndex(c *gin.Context) {
	...
	h.orm.Offset((page-1)*size).Limit(size).Find(&nodes, "team_id = ?", h.getCurrent(c).TeamID)
...
func (h Handler) NodeUpdate(c *gin.Context) {
	...
	h.orm.Where("id = ? AND team_id = ?", c.Param("id"), h.getCurrent(c).TeamID).Updates(&node).Scan(&node)
	c.JSON(http.StatusOK, node)
}
