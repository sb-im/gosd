...
function run(node_id)
  ...
  while( step_1 < 2 ) do
    ...
    {"|<-", ch_onDrone, function(ok, data)
      if data["result"] then
        step_1 = step_1 + 1
        print("Drone power_on_remote started")
      else
        print("EEEEEEEEEEE")
        print(json.encode(data))
        step_1 = err
    ...
    {"|<-", ch_onDoor, function(ok, data)
      if data["result"] then
          step_1 = step_1 + 1
          print("Drone power_on_drone started")
      else
        print("EEEEEEEEEEE")
        print(json.encode(data))
        step_1 = err
  ...
  -- local data, err = get_msg(depot_id, "weather")
  -- print(json.encode(data))
  ...
  if sync_call(drone_id, "power_off_drone") then
    return
  end

  if sync_call(drone_id, "power_off_remote_smart") then
    return
  end
